...
btn_multiply = tk.Button(root, text = "*", command= lambda: add_to_calculation("*"), width= 5, font= ("Ariel", 14))
btn_multiply.grid(row=4, column=4)

btn_divide = tk.Button(root, text = "/", command= lambda: add_to_calculation("/"), width= 5, font= ("Ariel", 14))
btn_divide.grid(row=5, column=4)

btn_openp = tk.Button(root, text = "(", command= lambda: add_to_calculation("("), width= 5, font= ("Ariel", 14))
btn_openp.grid(row=5, column=1)

btn_closep = tk.Button(root, text = ")", command= lambda: add_to_calculation(")"), width= 5, font= ("Ariel", 14))
btn_closep.grid(row=5, column=3)

btn_equals = tk.Button(root, text = "=", command= evaluate_calculation, width= 11, font= ("Ariel", 14))
btn_equals.grid(row=6, column=3, columnspan=2)

btn_clear = tk.Button(root, text = "Clear", command= clear_field, width= 11, font= ("Ariel", 14))
